...
from hey.mcp_tools.camel.toolkits import FunctionTool, VideoAnalysisToolkit
from hey.mcp_tools.camel.messages import BaseMessage
from hey.mcp_tools.camel.agents import ChatAgent
from hey.mcp_tools.camel.models import ModelFactory, BaseModelBackend
from hey.mcp_tools.camel.types import ModelType, ModelPlatformType
from hey.mcp_tools.camel.utils import dependencies_required
...
# 2. `click_blank_area()`: Click a blank area of the page to unfocus the current element. It is useful when you have clicked an element but it cannot unfocus itself (e.g. Menu bar) to automatically render the updated webpage.
# 3. `ask_question_about_video(question: str)`: Ask a question about the current webpage which contains video, e.g. youtube websites.
# """  # for debug
...
class BaseBrowser:
    def __init__(self, 
        ...
        self.playwright = None  # will be set in start()
    ...
    async def _wait_for_load(self, timeout: int = 20):
    ...
    async def click_blank_area(self):
    ...
    def ask_question_about_video(self, question: str) -> str:
        ...
        video_analyzer = VideoAnalysisToolkit(model=video_model)
    ...
    async def get_screenshot(self, save_image: bool = False) -> Tuple[Image.Image, Union[str, None]]:
    ...
    async def capture_full_page_screenshots(self, scroll_ratio: float = 0.8) -> List[str]:
    ...
    async def get_interactive_elements(self) -> List[Dict[str, Any]]:
        ...
        result = cast(
            ...
            Dict[str, Dict[str, Any]], await self.page.evaluate("MultimodalWebSurfer.getInteractiveRects();")
        )
    ...
    async def get_som_screenshot(self, save_image: bool = False) -> Tuple[Image.Image, Union[str, None]]:
    ...
    async def click_id(self, identifier: Union[str, int]):
    ...
    async def extract_url_content(self):
...
class AsyncWebToolkit(BaseToolkit):
...
You are a helpful planning agent that can assist users in planning complex tasks which need multi-step browser interaction.
    ...
    async def _observe(self, task_prompt: str, detailed_plan: Optional[str] = None) -> Tuple[str, str, str]:
...
```
        ...
        som_screenshot, som_screenshot_path = await self.browser.get_som_screenshot(save_image=True)
    ...
    async def _act(self, action_code: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # run or eval your code
            val = eval(code)
            # logging.info(f"[Zhifeng debug] eval({code!r}) -> {val!r} "
            #               f"(type={type(val)}, awaitable={inspect.isawaitable(val)}, "
            #               f"enter={hasattr(val, '__enter__')}, aenter={hasattr(val, '__aenter__')})")

            # 2) if it’s a coroutine, await it *first*
            if inspect.isawaitable(val):
                # logging.info(f"[debug] awaiting {code}…")
                val = await val

            # logging.info("[Zhifeng debug] after await, about to return—no context‐manager logic remains")
            # time.sleep(1)
            await asyncio.sleep(1)
            ...
            # time.sleep(1)
            await asyncio.sleep(1)
            err_msg = (f"Error while executing the action {action_code}: {e} (Type {type(e)})\n{traceback.format_stack()}. "
                       f"If timeout, please recheck whether you have provided the correct identifier.")
...
"""
    ...
    # Otherwise inspect.iscoroutinefunction returns False, i.e., the decorator makes it synchronous
    # def browser_simulation(self,
    async def browser_simulation(self,
                           task_prompt: str, 
                           start_url: str,
                           # round_limit: int = 12
                           ) -> str:
        ...
        for i in range(round_limit):
            ...
            observation, reasoning, action_code = await self._observe(task_prompt, detailed_plan)
